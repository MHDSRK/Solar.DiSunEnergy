...
export default function AdminPage() {
  ...
  const createFollowup = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (!detail || !followUpAt) return
    setFollowUpBusy(true)
    try {
      const response = await fetch(`/api/admin/leads/${encodeURIComponent(detail.lead_id)}/followups`, {
        method: 'POST', headers: {'content-type': 'application/json'},
        body: JSON.stringify({ followUpAt: new Date(followUpAt).toISOString(), note: followUpNote }),
      })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(data.message || 'Unable to create follow-up.')
      setFollowUpAt(''); setFollowUpNote(''); setMessage(`Follow-up scheduled for ${detail.lead_id}.`); await load()
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Unable to create follow-up.')
