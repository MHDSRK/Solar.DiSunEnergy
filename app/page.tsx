...
requestAnimationFrame(() => {
  ...
  const submitFeasibility = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const hasSectionOffice = feasibilityForm.sectionOffice.trim().length > 0
    const hasArea = feasibilityForm.landmark.trim().length > 0
    if (!hasConsumerNumber && !hasSectionOffice && !hasArea) nextErrors.lookup = 'Enter a Consumer Number, Section Office, or Area / Landmark.'
