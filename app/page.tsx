      ...
      {isCalculatorOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#03132f]/95 pt-[78px]" role="dialog" aria-modal="true" aria-labelledby="calculator-title">
          ...
          <section ref={sheetRef} className="relative isolate max-h-[calc(100dvh-94px)] w-[calc(100%-24px)] overflow-x-hidden overflow-y-auto rounded-[28px] bg-white px-4 pb-6 pt-2 text-[#071528] shadow-[0_12px_32px_rgba(0,0,0,.28)] animate-in slide-in-from-bottom duration-300">
            ...
            <h2 id="calculator-title" className="relative z-0 mt-4 px-2 text-center text-2xl font-extrabold leading-[1.05] tracking-[-.04em]">SOLAR POWER<br />CALCULATOR</h2>
            ...
            <form onSubmit={validateAndSubmit} className="border-x border-b border-[#8bd35c] px-3 py-2" noValidate><div className="grid grid-cols-2 items-start gap-3"><label className="block text-xs font-medium leading-tight">{calculationMode === 'bill' ? <>AVERAGE MONTHLY<br />ELECTRICITY BILL (₹)</> : <>AVERAGE UNIT MONTHLY<br />CONSUMED (KW)</>}<span className="relative mt-1 block"><span className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-sm">{calculationMode === 'bill' ? '₹' : 'KW'}</span><input value={form.bill} onChange={(event) => updateField('bill', event.target.value.replace(/\D/g, ''))} className={`h-10 w-full rounded-lg border pl-6 pr-8 text-sm outline-none focus:border-[#159600] ${errors.bill ? 'border-red-500' : 'border-slate-400'}`} inputMode="numeric" aria-invalid={Boolean(errors.bill)} onBlur={(event) => validateField('bill', event.target.value)} /><span className="absolute right-1 top-1/2 flex -translate-y-1/2 flex-col"><button type="button" aria-label={calculationMode === 'bill' ? 'Increase monthly bill' : 'Increase monthly units'} className="h-4 px-1 text-xs leading-none" onClick={() => updateField('bill', String(Number(form.bill || 0) + 100))}>▲</button><button type="button" aria-label={calculationMode === 'bill' ? 'Decrease monthly bill' : 'Decrease monthly units'} className="h-4 px-1 text-xs leading-none" onClick={() => updateField('bill', String(Math.max(0, Number(form.bill || 0) - 100)))}>▼</button></span></span>{errors.bill && <span className="mt-1 block text-[9px] font-normal text-red-600">{errors.bill}</span>}</label><label className="block text-xs font-medium leading-tight">CONNECTION<br />CATEGORY<select value={form.category} onChange={(event) => updateField('category', event.target.value)} onBlur={(event) => validateField('category', event.target.value)} className={`mt-1 h-10 w-full rounded-lg border bg-white px-2 text-sm outline-none focus:border-[#159600] ${errors.category ? 'border-red-500' : 'border-slate-400'}`} aria-invalid={Boolean(errors.category)}><option value="">Select</option><option value="domestic">Domestic</option><option value="commercial">Commercial</option></select>{errors.category && <span className="mt-1 block text-[9px] font-normal text-red-600">{errors.category}</span>}</label></div><p className="mt-1 whitespace-nowrap pt-0 text-[7px] leading-none tracking-[-0.03em] text-slate-500">{calculationMode === 'bill' ? 'Tip: If your KSEB bill is bi-monthly, divide the bill amount by 2 to get your monthly average.' : 'Tip: If your KSEB bill is bi-monthly, divide total units by 2 to get your monthly average.'}</p><div className="mt-3 border-t border-slate-200 pt-3"><div className="space-y-3"><label className="block text-xs font-medium">FULL NAME<input value={form.fullName} onChange={(event) => updateField('fullName', event.target.value)} onBlur={(event) => validateField('fullName', event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" aria-invalid={Boolean(errors.fullName)} />{errors.fullName && <span className="mt-1 block text-[9px] font-normal text-red-600">{errors.fullName}</span>}</label><label className="block text-xs font-medium">PHONE NUMBER<input value={form.phone} onChange={(event) => updateField('phone', event.target.value.replace(/\D/g, '').slice(0, 10))} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" inputMode="numeric" maxLength={10} aria-invalid={Boolean(errors.phone)} onBlur={(event) => validateField('phone', event.target.value)} />{errors.phone && <span className="mt-1 block text-[9px] font-normal text-red-600">{errors.phone}</span>}</label><div className="grid grid-cols-2 gap-3"><label className="block text-xs font-medium">DISTRICT<select value={form.district} onChange={(event) => updateField('district', event.target.value)} onBlur={(event) => validateField('district', event.target.value)} className={`mt-1 h-10 w-full rounded-lg border bg-white px-2 text-sm outline-none focus:border-[#159600] ${errors.district ? 'border-red-500' : 'border-slate-400'}`} aria-invalid={Boolean(errors.district)}><option value="">Select district</option>{keralaDistricts.map((district) => <option key={district} value={district}>{district}</option>)}</select>{errors.district && <span className="mt-1 block text-[9px] font-normal text-red-600">{errors.district}</span>}</label><label className="block text-xs font-medium">AREA<input value={form.area} onChange={(event) => updateField('area', event.target.value)} onBlur={(event) => validateField('area', event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" aria-invalid={Boolean(errors.area)} />{errors.area && <span className="mt-1 block text-[9px] font-normal text-red-600">{errors.area}</span>}</label></div><button type="submit" className="mt-5 flex min-h-11 w-full items-center justify-center rounded-full bg-[#1260a4] px-6 text-sm font-extrabold tracking-[0.08em] text-white shadow-none transition-transform hover:scale-[1.01]">SUBMIT</button>{isCalculating && <div className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-slate-50 px-3 py-4 text-xs font-semibold text-slate-500 animate-in fade-in"><span className="size-3 animate-spin rounded-full border-2 border-[#159600]/25 border-t-[#159600]" />Preparing your solar estimate...</div>}{result && <div className="mt-6 space-y-5 animate-in fade-in duration-500"><h3 className="text-center text-xl font-extrabold tracking-[-.03em]">Solar Estimate &amp; Financial Summary</h3><div className="rounded-[20px] border-2 border-slate-200 bg-white p-5"><div className="grid grid-cols-2 gap-4"><div><p className="text-xs font-bold uppercase text-slate-400">Recommended plant</p><p className="mt-2 text-3xl font-extrabold text-[#168566]">{result.kw} <span className="text-xl">kW</span></p></div><div className="text-right"><p className="text-xs font-bold uppercase text-slate-400">Approx. roof area</p><p className="mt-2 text-xl font-extrabold text-[#293244]">{result.roofMin}–{result.roofMax} sq ft</p></div></div></div><div className="rounded-[20px] border-2 border-slate-200 bg-white p-5 text-sm"><div className="flex justify-between gap-3"><span>Estimated Total Setup Cost:</span><strong>₹ {result.cost.toLocaleString('en-IN')}</strong></div><div className="mt-4 flex justify-between gap-3 text-[#168566]"><span>- PM Surya Ghar Govt Subsidy:<small className="block text-slate-400">(Central Govt Grant)</small></span><strong>-₹ {result.subsidy.toLocaleString('en-IN')}</strong></div><div className="mt-4 flex justify-between gap-3 text-[#168566]"><span>- Maximum Bank Loan:<small className="block text-slate-400">(Up to)</small></span><strong>-₹ {result.loan.toLocaleString('en-IN')}</strong></div><div className="mt-6 flex justify-between gap-3 text-lg font-extrabold"><span>Est. Out-of-Pocket Cost:</span><strong className="text-[#168566]">₹ {result.netCost.toLocaleString('en-IN')}</strong></div></div><div className="flex items-center justify-between rounded-[20px] border-2 border-slate-200 bg-white p-5 text-lg font-extrabold text-[#168566]"><span>Est. Annual Savings:</span><strong>₹ {(result.kw * 6800).toLocaleString('en-IN')}</strong></div></div>}<strong>Recommended system</strong><span className="text-right">{result.kw} kW</span><strong>Roof area</strong><span className="text-right">{result.roofMin}–{result.roofMax} sq ft</span><strong>Setup cost</strong><span className="text-right">₹{result.cost.toLocaleString('en-IN')}</span><strong>PM Surya Ghar subsidy</strong><span className="text-right">₹{result.subsidy.toLocaleString('en-IN')}</span><strong>Bank loan</strong><span className="text-right">₹{result.loan.toLocaleString('en-IN')}</span><strong>Estimated net cost</strong><span className="text-right">₹{result.netCost.toLocaleString('en-IN')}</span></div></div></div></div></form>
          </section>
        </div>
      )}
    </main>
