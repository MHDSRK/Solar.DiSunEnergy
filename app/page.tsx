...
export default function Page() {
  ...
  return (
    <main
      className="h-dvh overflow-y-auto overscroll-contain bg-[#02122d] text-white"
      onScroll={(event) => {
        const target = event.currentTarget
        setScrollProgress(Math.min(1, target.scrollTop / 180))
      }}
    >
      <section className="relative h-dvh bg-[#02122d]">
        ...
        <header className="absolute inset-x-0 top-0 z-30 flex h-[59px] origin-top items-center justify-between rounded-b-[22px] bg-white px-4 shadow-[0_8px_18px_rgba(0,0,0,.2)]" style={{ transform: `translateY(${-scrollProgress * 100}%) scale(${1 - scrollProgress * 0.18})`, opacity: 1 - scrollProgress * 0.12 }}>
        ...
        <div id="top" className="relative z-10 flex flex-col items-center px-6 pt-[160px] text-center">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 rounded-t-[23px] bg-white px-5 pb-8 pt-5 text-center text-[#071528] shadow-[0_-5px_18px_rgba(0,0,0,.22)]" style={{ transform: `translateY(${scrollProgress * 100}%)` }}>
      ...
      <section id="book" className="min-h-[420px] bg-white px-0 pb-8 pt-8"><div className="mx-auto flex max-w-[390px] items-center gap-3 rounded-t-[17px] border border-[#d9dde5] bg-white px-8 py-4 text-[#182433] shadow-[0_-5px_13px_rgba(0,0,0,.25)]"><span className="text-3xl text-[#e7bf2e]">☀</span><div className="flex-1"><h2 className="text-[14px] font-bold">Ready to Go Solar?</h2><p className="text-[10px] text-[#555]">Book your free site visit Now</p></div><a href="mailto:hello@solar.com" className="rounded-full border-2 border-[#62a82a] bg-[#36a21d] px-3 py-2 text-[10px] font-bold text-white shadow-[inset_0_0_0_2px_white]">BOOK VISIT &gt;</a></div></section>
    </main>
  )
}
