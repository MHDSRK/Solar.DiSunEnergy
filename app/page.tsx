          ...
          <section ref={sheetRef} className="relative isolate max-h-[calc(100dvh-94px)] w-[calc(100%-24px)] overflow-x-hidden overflow-y-auto rounded-[28px] bg-white px-4 pb-6 pt-2 text-[#071528] shadow-[0_12px_32px_rgba(0,0,0,.28)] animate-in slide-in-from-bottom duration-300">
            ...
            <form onSubmit={validateAndSubmit} className="border-x border-b border-[#8bd35c] px-3 py-2" noValidate>
              ...
              <div className="mt-3 border-t border-slate-200 pt-3"><div className="space-y-3">
                ...
                {result && <div ref={resultRef} className="mt-1 scroll-mt-12 space-y-3 animate-in fade-in duration-500"><h3 className="-mt-1 text-center text-base font-extrabold leading-tight">Solar Estimate &amp; Financial Summary</h3><div className="rounded-2xl border-2 border-slate-200 bg-white p-4 text-center"><div className="border-b border-slate-200 pb-3"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Recommended plant</p><p className="mt-1 text-3xl font-extrabold text-[#168566]">{result.kw} <span className="text-lg">kW</span></p></div><div className="pt-3"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400">Approx. roof area</p><p className="mt-1 text-xl font-extrabold text-[#293244]">{result.roofMin}–{result.roofMax} sq ft</p></div></div><div className="rounded-2xl border-2 border-slate-200 bg-white p-3 text-[10px]"><div className="flex justify-between gap-2"><span>Estimated Total Setup Cost:</span><strong>₹ {result.cost.toLocaleString('en-IN')}</strong></div><div className="mt-2 flex justify-between gap-2 text-[#168566]"><span>- PM Surya Ghar Govt Subsidy:<small className="block text-slate-400">(Central Govt Grant)</small></span><strong>-₹ {result.subsidy.toLocaleString('en-IN')}</strong></div><div className="mt-2 flex justify-between gap-2 text-[#168566]"><span>- Maximum Bank Loan:<small className="block text-slate-400">(Up to)</small></span><strong>-₹ {result.loan.toLocaleString('en-IN')}</strong></div><div className="mt-3 flex justify-between gap-2 text-sm font-extrabold"><span>Est. Out-of-Pocket Cost:</span><strong className="text-[#168566]">₹ {result.netCost.toLocaleString('en-IN')}</strong></div></div><div className="flex items-center justify-between rounded-2xl border-2 border-slate-200 bg-white p-3 text-sm font-extrabold text-[#168566]"><span>Est. Annual Savings:</span><strong>₹ {(result.kw * 6800).toLocaleString('en-IN')}</strong></div><button type="button" className="mt-3 flex min-h-11 w-full items-center justify-center gap-2 rounded-full border-2 border-[#1260a4] bg-white px-6 text-sm font-extrabold tracking-[0.08em] text-[#1260a4] shadow-none">CHECK FEASIBILITY <span aria-hidden="true" className="text-lg leading-none">→</span></button></div>}
              </div></div>
            </form>
          </section>
        </div>
