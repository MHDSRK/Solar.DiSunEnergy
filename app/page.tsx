      ...
      {isCalculatorOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#03132f]/95 pt-[78px]" role="dialog" aria-modal="true" aria-labelledby="calculator-title">
          ...
          <section ref={sheetRef} className="relative isolate max-h-[calc(100dvh-94px)] w-[calc(100%-24px)] overflow-x-hidden overflow-y-auto rounded-[28px] bg-white px-4 pb-6 pt-2 text-[#071528] shadow-[0_12px_32px_rgba(0,0,0,.28)] animate-in slide-in-from-bottom duration-300">
            ...
            {isFeasibilityPage && <form onSubmit={(event) => event.preventDefault()} className="px-2 pb-4 pt-8 text-center"><h2 className="text-2xl font-extrabold leading-tight tracking-[-.04em]">CHECK TRANSFORMER<br />FEASIBILITY</h2><p className="mt-3 text-sm leading-relaxed text-slate-500">Review your solar estimate and continue with a feasibility check for your property.</p><div className="mt-6 space-y-3 text-left"><label className="block text-[10px] font-medium">KSEB CONSUMER NUMBER<input value={feasibilityForm.consumerNumber} onChange={(event) => setFeasibilityForm((current) => ({ ...current, consumerNumber: event.target.value }))} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" required /></label><div className="grid grid-cols-2 gap-3"><label className="block text-[10px] font-medium">KSEB SECTION OFFICE<input value={feasibilityForm.sectionOffice} onChange={(event) => setFeasibilityForm((current) => ({ ...current, sectionOffice: event.target.value }))} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" required /></label><label className="block text-[10px] font-medium">AREA / LANDMARK<input value={feasibilityForm.landmark} onChange={(event) => setFeasibilityForm((current) => ({ ...current, landmark: event.target.value }))} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" required /></label></div><label className="block text-[10px] font-medium">TRANSFORMER IDENTIFICATION NAME (IF AVAILABLE)<input value={feasibilityForm.transformerName} onChange={(event) => setFeasibilityForm((current) => ({ ...current, transformerName: event.target.value }))} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" /></label></div><button type="submit" className="mt-6 flex min-h-11 w-full items-center justify-center rounded-full bg-[#1260a4] px-6 text-sm font-extrabold tracking-[0.08em] text-white">SUBMIT</button></form>}
          </section>
        </div>
      )}
    </main>
