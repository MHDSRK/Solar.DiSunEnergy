...
export default function Page() {
  ...
  const [ksebTransformers, setKsebTransformers] = useState<Array<{ id: string; name: string; balanceAvailableKw: number }>>([])
...
requestAnimationFrame(() => {
  ...
  const selectKsebDistrict = async (districtId: string) => {
    ...
    setFeasibilityForm((current) => ({ ...current, districtId, districtName: district?.name ?? '', sectionId: '', sectionOffice: '', transformerName: '' }))
  ...
  const submitFeasibility = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const nextErrors: Record<string, string> = {}
    const hasConsumerNumber = feasibilityForm.consumerNumber.trim().length > 0
    ...
    if (hasConsumerNumber && !/^\d{13}$/.test(feasibilityForm.consumerNumber.replace(/[\s-]/g, ''))) nextErrors.consumerNumber = 'Please enter a valid 13-digit KSEB Consumer Number.'
    setFeasibilityErrors(nextErrors)
    if (Object.keys(nextErrors).length) return
    setIsCheckingFeasibility(true)
    setFeasibilityResult(null)
    try {
      const response = await fetch('/api/kseb/transformer-feasibility', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ consumerNumber: feasibilityForm.consumerNumber, district: feasibilityForm.districtName, sectionId: feasibilityForm.sectionId, sectionOffice: feasibilityForm.sectionOffice, transformerId: feasibilityForm.transformerId, transformerName: feasibilityForm.transformerName, requestedKw: result?.kw }) })
      ...
      {isCalculatorOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#03132f]/95 pt-[78px]" role="dialog" aria-modal="true" aria-labelledby="calculator-title">
          ...
          <section ref={sheetRef} className="relative isolate max-h-[calc(100dvh-94px)] w-[calc(100%-24px)] overflow-x-hidden overflow-y-auto rounded-[28px] bg-white px-4 pb-6 pt-2 text-[#071528] shadow-[0_12px_32px_rgba(0,0,0,.28)] animate-in slide-in-from-bottom duration-300">
            ...
            {isFeasibilityPage && <form onSubmit={submitFeasibility} className="px-2 pb-4 pt-8 text-center"><h2 className="text-2xl font-extrabold leading-tight tracking-[-.04em]">CHECK TRANSFORMER<br />FEASIBILITY</h2>{feasibilityErrors.lookup && <p className="mb-3 text-left text-[9px] font-normal text-red-600">{feasibilityErrors.lookup}</p>}<div className="mt-6 space-y-3 text-left"><label className="block text-[10px] font-medium">TRANSFORMER NAME<select value={feasibilityForm.transformerId} disabled={!ksebTransformers.length || isCheckingFeasibility} onChange={(event) => { const selected = ksebTransformers.find((item) => item.id === event.target.value); setFeasibilityForm((current) => ({ ...current, transformerId: event.target.value, transformerName: selected?.name ?? '' })); setFeasibilityResult(null) }} className="mt-1 h-10 w-full rounded-lg border border-slate-400 bg-white px-2 text-sm outline-none disabled:bg-slate-100"><option value="">{isCheckingFeasibility ? 'Loading transformers...' : ksebTransformers.length ? 'Select Transformer' : 'Select Section First'}</option>{ksebTransformers.map((transformer) => <option key={transformer.id} value={transformer.id}>{transformer.name} — {transformer.balanceAvailableKw} kW available</option>)}</select></label>{feasibilityErrors.transformer && <p className="text-[9px] font-normal text-red-600">{feasibilityErrors.transformer}</p>}<label className="block text-[10px] font-medium">KSEB CONSUMER NUMBER<input value={feasibilityForm.consumerNumber} onChange={(event) => { setFeasibilityForm((current) => ({ ...current, consumerNumber: event.target.value })); setFeasibilityErrors((current) => ({ ...current, consumerNumber: '', lookup: '' })) }} onBlur={() => { const value = feasibilityForm.consumerNumber.replace(/[\s-]/g, ''); if (value && !/^\d{13}$/.test(value)) setFeasibilityErrors((current) => ({ ...current, consumerNumber: 'Enter a valid 13-digit consumer number' })) }} className={`mt-1 h-10 w-full rounded-lg border ${feasibilityErrors.consumerNumber ? 'border-red-500' : 'border-slate-400'} px-2 text-sm outline-none focus:border-[#159600]`} />{feasibilityErrors.consumerNumber && <span className="mt-1 block text-[9px] font-normal text-red-600">{feasibilityErrors.consumerNumber}</span>}</label><div className="space-y-3"><label className="block text-[10px] font-medium">NAME OF DISTRICT<select value={feasibilityForm.districtId} onFocus={loadKsebDistricts} onChange={(event) => selectKsebDistrict(event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-slate-400 bg-white px-2 text-sm outline-none focus:border-[#159600]"><option value="">{isLoadingDistricts ? 'Loading districts...' : 'Select District'}</option>{ksebDistricts.map((district) => <option key={district.id} value={district.id}>{district.name}</option>)}</select></label><label className="block text-[10px] font-medium">NAME OF SECTION<select value={feasibilityForm.sectionId} disabled={!feasibilityForm.districtId || isLoadingSections} onChange={(event) => selectKsebSection(event.target.value)} className="mt-1 h-10 w-full rounded-lg border border-slate-400 bg-white px-2 text-sm outline-none disabled:bg-slate-100 focus:border-[#159600]"><option value="">{isLoadingSections ? 'Loading sections...' : feasibilityForm.districtId ? 'Select Section' : 'Select District First'}</option>{ksebSections.map((section) => <option key={section.id} value={section.id}>{section.name}</option>)}</select></label><label className="block text-[10px] font-medium">AREA / LANDMARK<input value={''} onChange={(event) => { setFeasibilityForm((current) => ({ ...current, landmark: event.target.value })); setFeasibilityErrors((current) => ({ ...current, lookup: '' })) }} className={`mt-1 h-10 w-full rounded-lg border px-2 text-sm outline-none focus:border-[#159600] ${feasibilityErrors.landmark ? 'border-red-500' : 'border-slate-400'}`} />{feasibilityErrors.landmark && <span className="mt-1 block text-[9px] font-normal text-red-600">{feasibilityErrors.landmark}</span>}</label></div><label className="block text-[10px] font-medium">TRANSFORMER IDENTIFICATION NAME (IF AVAILABLE)<input value={feasibilityForm.transformerName} onChange={(event) => setFeasibilityForm((current) => ({ ...current, transformerName: event.target.value }))} className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" /></label></div><button type="submit" disabled={isCheckingFeasibility} className="mt-6 flex min-h-11 w-full items-center justify-center rounded-full bg-[#1260a4] px-6 text-sm font-extrabold tracking-[0.08em] text-white disabled:opacity-60">{isCheckingFeasibility ? 'CHECKING KSEB CAPACITY...' : 'CHECK FEASIBILITY'}</button>{isCheckingFeasibility && <div className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-slate-50 px-3 py-4 text-xs font-semibold text-slate-500"><span className="size-3 animate-spin rounded-full border-2 border-[#1260a4]/25 border-t-[#1260a4]" />Checking transformer feasibility...</div>}{feasibilityResult && <div className={`mt-4 rounded-2xl border-2 p-4 text-left text-xs ${feasibilityResult.success ? 'border-[#8bd35c]' : 'border-amber-300'}`}><h3 className="text-base font-extrabold">{Boolean(feasibilityResult.success) ? String((feasibilityResult.feasibility as { status?: string })?.status ?? '').replaceAll('_', ' ') : String(feasibilityResult.message ?? 'KSEB capacity data is unavailable.')}</h3>{Boolean(feasibilityResult.success) && <div className="mt-3 space-y-2 leading-relaxed"><p><strong>Section Office</strong><br />{String((feasibilityResult.section as { name?: string })?.name ?? '')}</p><p><strong>Area</strong><br />{String(feasibilityResult.area ?? '')}</p><p><strong>Transformer</strong><br />{String((feasibilityResult.transformer as { transformerName?: string })?.transformerName ?? '')}</p><p><strong>Feeder</strong><br />{String((feasibilityResult.transformer as { feederName?: string })?.feederName ?? '')}</p><div className="grid grid-cols-2 gap-2 border-t border-slate-200 pt-2"><span>90% DTR Capacity<br /><strong>{String((feasibilityResult.transformer as { allowedCapacityKw?: number })?.allowedCapacityKw)} kW</strong></span><span>Feasibility Issued<br /><strong>{String((feasibilityResult.transformer as { feasibilityIssuedKw?: number })?.feasibilityIssuedKw)} kW</strong></span><span>Grid Connected<br /><strong>{String((feasibilityResult.transformer as { gridConnectedKw?: number })?.gridConnectedKw)} kW</strong></span><span>Balance Available<br /><strong>{String((feasibilityResult.transformer as { balanceAvailableKw?: number })?.balanceAvailableKw)} kW</strong></span></div><p className="border-t border-slate-200 pt-2"><strong>Requested Solar Capacity</strong><br />{String(feasibilityResult.requestedKw)} kW<br /><strong>Remaining Capacity After Proposed System</strong><br />{String(feasibilityResult.remainingAfterInstallationKw)} kW</p><p className={feasibilityResult.status === 'PRELIMINARILY_FEASIBLE' ? 'text-[#168566]' : 'text-red-600'}><strong>{feasibilityResult.status === 'PRELIMINARILY_FEASIBLE' ? 'CAPACITY APPEARS AVAILABLE' : 'INSUFFICIENT TRANSFORMER CAPACITY'}</strong></p></div>}<p className="mt-3 text-[10px] text-slate-500">This is a preliminary capacity check based on publicly retrieved KSEB transformer data. Final feasibility and approval remain subject to KSEB&apos;s official process.</p></div>}</form>}
          </section>
        </div>
      )}
    </main>
