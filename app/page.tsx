    ...
    <main className="relative min-h-dvh overflow-y-auto bg-[#03132f] text-white" aria-label="DiSun Energy International solar calculator">
      ...
      {isCalculatorOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#03132f]/95 pt-[78px]" role="dialog" aria-modal="true" aria-labelledby="calculator-title">
          ...
          <section className="relative max-h-[calc(100dvh-94px)] w-[calc(100%-24px)] overflow-y-auto rounded-[28px] bg-white px-4 pb-6 pt-4 text-[#071528] shadow-[0_12px_32px_rgba(0,0,0,.28)] animate-in slide-in-from-bottom duration-300">
            <div className="mx-auto mb-3 h-1.5 w-12 rounded-full bg-slate-300" />
            <div className="sticky top-0 z-20 -mx-4 flex items-center justify-between bg-white px-4 pb-2 pt-0"><span className="text-sm font-semibold text-[#071528]">1 / 4</span><div className="flex items-center gap-1.5" aria-label="Step 1 of 4"><span className="h-1.5 w-8 rounded-full bg-[#1260a4]" /><span className="h-1.5 w-8 rounded-full bg-slate-300" /><span className="h-1.5 w-8 rounded-full bg-slate-300" /><span className="h-1.5 w-8 rounded-full bg-slate-300" /></div><button type="button" onClick={() => setIsCalculatorOpen(false)} aria-label="Close calculator" className="grid size-11 place-items-center text-3xl font-light">×</button></div>
            <h2 id="calculator-title" className="mt-2 text-center text-2xl font-extrabold leading-[1.05] tracking-[-.04em]">SUBSIDY &amp; SAVINGS<br />CALCULATOR</h2>
            <p className="mt-7 text-center text-lg font-bold">CALCULATION BASED ON</p>
            ...
            <div className="border-x border-b border-[#8bd35c] px-3 py-4"><div className="grid grid-cols-2 items-start gap-3"><label className="block text-[10px] font-medium leading-tight">AVERAGE MONTHLY<br />ELECTRICITY BILL (₹)<input className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" inputMode="numeric" /></label><label className="block text-[10px] font-medium leading-tight">CONNECTION<br />CATEGORY<input className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" /></label></div><p className="border-t border-slate-200 pt-5 text-xs leading-5 text-slate-500">Tip: If your KSEB bill is bi-monthly, divide the bill amount by 2 to get your monthly average.</p><div className="space-y-3"><label className="block text-[10px] font-medium">FULL NAME<input className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" /></label><label className="block text-[10px] font-medium">PHONE NUMBER<input className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" inputMode="tel" /></label><div className="grid grid-cols-2 gap-3"><label className="block text-[10px] font-medium">DISTRICT<input className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" /></label><label className="block text-[10px] font-medium">AREA<input className="mt-1 h-10 w-full rounded-lg border border-slate-400 px-2 text-sm outline-none focus:border-[#159600]" /></label></div></div></div>
          </section>
        </div>
      )}
    </main>
