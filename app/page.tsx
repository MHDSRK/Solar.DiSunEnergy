      ...
      {isCalculatorOpen && (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#03132f]/95 pt-[78px]" role="dialog" aria-modal="true" aria-labelledby="calculator-title">
          ...
          <section ref={sheetRef} className="relative isolate max-h-[calc(100dvh-94px)] w-[calc(100%-24px)] overflow-x-hidden overflow-y-auto rounded-[28px] bg-white px-4 pb-6 pt-2 text-[#071528] shadow-[0_12px_32px_rgba(0,0,0,.28)] animate-in slide-in-from-bottom duration-300">
            ...
            {isFeasibilityPage && <div className="px-2 pb-4 pt-8 text-center"><h2 className="text-2xl font-extrabold tracking-[-.04em]">CHECK FEASIBILITY</h2><p className="mt-3 text-sm leading-relaxed text-slate-500">Review your solar estimate and continue with a feasibility check for your property.</p><div className="mt-8 rounded-2xl border border-slate-200 p-5 text-left"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Your estimated system</p><p className="mt-2 text-3xl font-extrabold text-[#168566]">{result?.kw ?? 0} kW</p><p className="mt-1 text-sm text-slate-500">{result?.roofMin ?? 0}–{result?.roofMax ?? 0} sq ft roof area</p></div><button type="button" className="mt-6 flex min-h-11 w-full items-center justify-center rounded-full bg-[#1260a4] px-6 text-sm font-extrabold tracking-[0.08em] text-white">CONTINUE <span className="ml-2 text-lg">→</span></button></div>}
          </section>
        </div>
      )}
    </main>
